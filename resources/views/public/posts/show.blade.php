@php
    $shareMeta = \App\Support\ShareMetadata::forPost($post, $siteSetting);
@endphp

<x-layouts.public :title="$post->title" :meta="$shareMeta">
    @php
        $featuredImageUrl = filled($post->featured_image_path) ? asset('storage/'.$post->featured_image_path) : null;
    @endphp
        ...
        <main class="mx-auto max-w-6xl px-6 py-16 lg:px-8 lg:py-24">
            <div class="grid gap-10 lg:grid-cols-[minmax(0,1fr)_320px]">
                <article class="rounded-[2rem] border border-emerald-100 bg-white/90 p-8 shadow-xl shadow-emerald-900/5 transition-colors duration-300 dark:border-emerald-900/30 dark:bg-zinc-900/88 dark:shadow-black/20 lg:p-12">
                    <p class="text-sm font-semibold tracking-[0.24em] text-emerald-700 uppercase dark:text-emerald-300">{{ $post->category?->name ?? 'Berita' }}</p>
                    <h1 class="mt-4 text-2xl font-semibold text-zinc-950 text-balance dark:text-white lg:text-3xl">{{ $post->title }}</h1>
                    <div class="mt-5 flex flex-wrap items-center gap-4 text-sm text-zinc-500 dark:text-zinc-400">
                        ...
                        <span>{{ $post->author?->name ?? 'Editor Pesantren' }}</span>
                    </div>
                    ...
                    <div class="rich-content mt-8 text-base leading-8 text-zinc-700 dark:text-zinc-200">
                        <p class="text-lg leading-8 text-zinc-600 dark:text-zinc-300">{{ $post->excerpt }}</p>
                        {!! \App\Support\RichText::render($post->content) !!}
                    </div>
                </article>
                ...
                <aside class="space-y-6">
                    ...
                    <div class="rounded-[1.75rem] border border-emerald-100 bg-white p-6 shadow-sm transition-colors duration-300 dark:border-emerald-900/30 dark:bg-zinc-900">
                        <p class="text-sm font-semibold tracking-[0.2em] text-emerald-700 uppercase dark:text-emerald-300">Berita Lainnya</p>
                        <div class="mt-4 grid gap-4">
                            @foreach ($relatedPosts as $relatedPost)
                                <a href="{{ route('berita.show', $relatedPost) }}" class="rounded-2xl bg-emerald-50/80 p-4 transition hover:bg-emerald-100 dark:bg-zinc-800 dark:hover:bg-zinc-700 hover:text-emerald-700 dark:hover:text-emerald-300">
                                    <p class="text-sm font-semibold text-zinc-900 dark:text-white">{{ $relatedPost->title }}</p>
                                    <p class="mt-2 text-xs leading-6 text-zinc-500 dark:text-zinc-400">{{ optional($relatedPost->published_at)?->translatedFormat('d M Y') }}</p>
                                </a>
                            @endforeach
                        </div>
                    </div>
