    ...
    <div class="relative overflow-x-hidden">
        ...
        <main class="mx-auto max-w-6xl px-6 py-16 lg:px-8 lg:py-24">
            <div class="grid gap-10 lg:grid-cols-[minmax(0,1fr)_320px]">
                <article class="rounded-[2rem] border border-emerald-100 bg-white/90 p-8 shadow-xl shadow-emerald-900/5 transition-colors duration-300 dark:border-emerald-900/30 dark:bg-zinc-900/88 dark:shadow-black/20 lg:p-12">
                    <p class="text-sm font-semibold tracking-[0.24em] text-emerald-700 uppercase dark:text-emerald-300">{{ $studentWork->category?->name ?? 'Karya' }}</p>
                    <h1 class="mt-4 text-2xl font-semibold text-zinc-950 text-balance dark:text-white lg:text-3xl">{{ $studentWork->title }}</h1>
                    <div class="mt-5 flex flex-wrap items-center gap-4 text-sm text-zinc-500 dark:text-zinc-400">
                        ...
                        <span>{{ optional($studentWork->published_at)?->translatedFormat('d F Y') }}</span>
                        <span>{{ $studentWork->uploader?->name ?? 'Editor Pesantren' }}</span>
                    </div>
                    ...
                    <div class="rich-content mt-8 text-base leading-8 text-zinc-700 dark:text-zinc-200">
                        <p class="text-lg leading-8 text-zinc-600 dark:text-zinc-300">{{ $studentWork->excerpt }}</p>
                        {!! \App\Support\RichText::render($studentWork->content) !!}
                    </div>

                    @if ($attachmentUrl)
                        <div class="mt-8">
                            <a href="{{ $attachmentUrl }}" target="_blank" class="inline-flex rounded-full bg-emerald-950 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-emerald-800">
                                Buka Lampiran Karya
                            </a>
                        </div>
                    @endif
                ...
                <aside class="space-y-6">
                    ...
                    <div class="rounded-[1.75rem] border border-emerald-100 bg-white p-6 shadow-sm transition-colors duration-300 dark:border-emerald-900/30 dark:bg-zinc-900">
                        <p class="text-sm font-semibold tracking-[0.2em] text-emerald-700 uppercase dark:text-emerald-300">Karya Terkait</p>
                        <div class="mt-4 grid gap-4">
                            @foreach ($relatedWorks as $relatedWork)
                                <a href="{{ route('karya-santri.show', $relatedWork) }}" class="rounded-2xl bg-emerald-50/80 p-4 transition hover:bg-emerald-100 dark:bg-zinc-800 dark:hover:bg-zinc-700">
                                    <p class="text-sm font-semibold text-zinc-900 dark:text-white">{{ $relatedWork->title }}</p>
                                    <p class="mt-2 text-xs leading-6 text-zinc-500 dark:text-zinc-400">{{ $relatedWork->author_name }}</p>
                                </a>
                            @endforeach
                        </div>
                    </div>
                    ...
                    <section class="rounded-[1.75rem] border border-zinc-100 bg-white/80 p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900/80">
                        ...
                        <p class="mt-3 text-sm leading-6 text-zinc-600 dark:text-zinc-400">
                            Belum ada karya terkait lainnya.
                        </p>
                        <a
